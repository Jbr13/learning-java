package fundamentals.exercises.Char;
...
//        SqueakyClean.clean("à-ḃç");
    ...
    private static String kebabCaseToCamelCase(String str) {
        ...
        String result = String.format("%s%s%s", str01, str02.substring(0, 1).toUpperCase(), str02.substring(1, str02.length()));

        return result;
    }

    private static String omitCharactersThatAreNotLetters(String str) {
        return str.chars().filter(Character::isLetter).mapToObj(c -> String.valueOf(c)).collect(Collectors.joining());
    }
