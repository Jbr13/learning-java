package Dematic.IQ.entityBeans;
...
public class Validation {
    ...
    */
    ...
    /*
        ...
        @Constraint(validatedBy = CepValidator.class): Indicates that this annotation is a constraint annotation and specifies the class (CepValidator.class) responsible for validating this constraint.
        ...
        @Retention(RetentionPolicy.RUNTIME): Specifies the retention policy of the annotation, allowing it to be accessible at runtime via reflection.

        public @interface Cep {: Begins the definition of the custom annotation named Cep.

        String message() default "Invalid Cep";: Defines an element message within the annotation, providing a default error message for the validation failure.

        Class<?>[] groups() default { };: Defines an element groups, which can be used to specify validation groups (used in validation scenarios where groups of constraints are validated selectively).
        Class<? extends Payload>[] payload() default { };: Defines an element payload, typically used for additional metadata and validation-related information.
            Groups and payload are just used to
    */
